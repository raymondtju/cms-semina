...
function EventsEdit() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await updateData(`/cms/events/${eventId}`, payload);
    if (res.data.data) {
      dispatch(
        setAlert(true, "success", `berhasil ubah events ${res.data.data.title}`)
      );
  ...
  return (
    <Container>
      <BreadCrumb second={"Events"} secondUrl={"/events"} third="Edit" />
      {alerts.status && <Alert type={alerts.type} message={alerts.message} />}
      <Form
        form={form}
        isLoading={isLoading}
        lists={lists}
